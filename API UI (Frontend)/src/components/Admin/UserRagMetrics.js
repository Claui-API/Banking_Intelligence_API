...
/**
 * Component to display enhanced per-user RAG metrics for admin analytics
 */
const UserRagMetrics = () => {
  ...
  // Fetch user metrics with enhanced data
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        
        // Fetch user metrics with enhanced flag
        ...
        logger.info(`Loaded ${useEnhanced ? 'enhanced' : 'basic'} user RAG metrics`);
      } catch (err) {
      ...
      .sort((a, b) => {
        ...
        switch(sortBy) {
          ...
          case 'queryCount':
            ...
            break;
          case 'cacheHitRate':
            comparison = parseFloat(a.cacheHitRate || 0) - parseFloat(b.cacheHitRate || 0);
            break;
          case 'lastActive':
  ...
  const prepareActivityData = (user) => {
    ...
    const hourlyData = user.activityByHour.map((count, hour) => ({
      hour: `${hour}:00`,
      queries: count
    }));
    
    const days = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
    const dailyData = user.activityByDay.map((count, day) => ({
      day: days[day],
      queries: count
    }));
      ...
      <>
        <div className="d-flex justify-content-between align-items-center mb-3">
          ...
          <Form.Check
            ...
            checked={useEnhanced}
            onChange={() => setUseEnhanced(!useEnhanced)}
          />
        </div>
          ...
          <Table responsive striped hover className='table-light'>
            <thead>
              <tr>
                ...
                <th onClick={() => setSortBy('cacheHitRate')}>Cache Hit Rate</th>
      ...
      <>
        ...
        <Row className="mb-4">
          ...
          <Col md={3}>
            <Card className="h-100">
              <Card.Body className="text-center">
                ...
                <h2>{selectedUser.cacheHitRate}%</h2>
              </Card.Body>
            </Card>
          </Col>
          <Col md={3}>
          ...
          <Col md={3}>
            <Card className="h-100">
              <Card.Body className="text-center">
                <h6 className="text-muted">Cost Savings</h6>
                <h2>${selectedUser.costSavings}</h2>
              </Card.Body>
            </Card>
          </Col>
        </Row>
                ...
                <tbody>
                  {selectedUser.recentQueries.map((query, index) => (
                    <tr key={index}>
                      ...
                      <td>
                        {query.cachedResponse ? 
                          <Badge bg="success">Cache</Badge> : 
                          <Badge bg="primary">API</Badge>}
                      </td>
